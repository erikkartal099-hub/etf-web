...
test.describe('AI Real-time Intelligence - Complete Chat Testing', () => {
  ...
  test('should test complete AI chat flow with intelligent responses', async ({ page }) => {
    ...
    // Find and click send button (using correct selector from ChatWidget)
    const sendButton = page.locator('button:has(svg)').last(); // Last button in the input area
    ...
    const aiResponseElements = page.locator('div:has-text("ETF")').filter({ hasNotText: 'What is the CoinDesk' });
    const hasAIResponse = await aiResponseElements.count() > 0;
